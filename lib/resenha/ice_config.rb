...
module Resenha
  ...
  # room join. TURN servers come from two independent lists: resenha_turn_servers
  # uses the static username/password pair, while resenha_turn_secret_servers
  ...
  module IceConfig
    ...
    def self.static_turn_servers
      username = SiteSetting.resenha_turn_username
      credential = SiteSetting.resenha_turn_credential
    ...
    def self.ephemeral_turn_servers(user)
      return [] if ephemeral_turn_urls.empty? || SiteSetting.resenha_turn_secret.blank?
    ...
    def self.transport_policy
      ephemeral_active = ephemeral_turn_urls.any? && SiteSetting.resenha_turn_secret.present?
    ...
    def self.stun_urls
      parse_url_list(SiteSetting.resenha_stun_servers)
    end

    def self.static_turn_urls
      parse_url_list(SiteSetting.resenha_turn_servers)
    end

    def self.ephemeral_turn_urls
      parse_url_list(SiteSetting.resenha_turn_secret_servers)
    end
