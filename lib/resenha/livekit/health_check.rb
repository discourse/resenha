...
module Resenha
  module Livekit
    ...
    module HealthCheck
      LAST_PROBE_KEY = "resenha:livekit:last_probe"
      LAST_PROBE_TTL = 7.days

      def self.settings_status
        {
          url_present: SiteSetting.resenha_livekit_url.present?,
          api_key_present: SiteSetting.resenha_livekit_api_key.present?,
          api_secret_present: SiteSetting.resenha_livekit_api_secret.present?,
          policy: SiteSetting.resenha_livekit_room_policy,
        }
      end
      ...
      def self.token_check
        ...
        JWT.decode(token, SiteSetting.resenha_livekit_api_secret, true, algorithm: "HS256")
        { ok: true }
      ...
      def self.pinned_rooms_status(probe_participants: true)
        room_ids =
      ...
      def self.room_status(room, probe_participants:)
        status = {
          room_id: room.id,
          name: room.name,
