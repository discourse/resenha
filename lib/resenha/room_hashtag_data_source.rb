...
module Resenha
  class RoomHashtagDataSource
    ...
    def self.find_by_ids(guardian, ids)
      visible_rooms(guardian, Resenha::Room.where(id: ids)).map do |room|
        room_to_hashtag_item(room)
      end
    end
    ...
    def self.lookup(guardian, slugs)
      ...
      visible_rooms(
        guardian,
        Resenha::Room.where("LOWER(slug) IN (?)", slugs.map(&:downcase)),
    ...
    def self.search(
      ...
      visible_rooms(
        guardian,
        Resenha::Room.where("slug ILIKE :pattern OR name ILIKE :pattern", pattern: pattern),
    ...
    def self.search_without_term(guardian, limit)
      visible_rooms(guardian, Resenha::Room.all)
        .take(limit)
        .map { |room| room_to_hashtag_item(room) }
    end
