...
function roomForThread(threadId) {
  if (!roomByThread.has(threadId)) {
    roomByThread.set(
      threadId,
      ajax(`/resenha/chat_threads/${threadId}.json`).catch((error) => {
...
export default class ResenhaBackToVoiceRoomButton extends Component {
  @service router;
  ...
  get shouldLookup() {
    return (
      ...
      this.siteSettings.resenha_chat_enabled &&
  ...
  async openRoom(room) {
    const chatActive = await ajax(
      `/resenha/chat_threads/${this.thread.id}.json`
    ).then(
        ...
        <:content as |room|>
          {{#if room}}
            <DButton
              ...
              @translatedTitle={{i18n "resenha.chat.back_to_voice_room"}}
