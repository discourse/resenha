...
`,Zu=e=>`quick_gelu_impl(${e})`,oh=(e,t)=>{let r=Ge(e.inputs[0].dataType);e.compute(_e(e.inputs[0],"QuickGelu",Zu,Xu(r,t.alpha),t.cacheKey,e.inputs[0].dataType))}}),J0=L(()=>{"use strict";ie(),se(),js(),Yu=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},Ju=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let r=R("input",e[0].dataType,e[0].dims,4),n=R("bias",e[0].dataType,[e[0].dims[2]],4),i=Z("output",e[0].dataType,t,4),s=O.size(t)/4,a=De(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)}}),getShaderSource:u=>`
...
`)};
    ...
    ${_}`},p=[{type:12,data:l},...te(t,n)];return{name:"Expand",shaderCache:{hint:`${n.length};${a}${u}`,inputDependencies:["rank"]},getShaderSource:d,getRunData:()=>({outputs:[{dims:n,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(l/64)},programUniforms:p})}},Dh=e=>{Ml(e.inputs),e.compute(Nl(e.inputs),{inputs:[0]})}}),h_=L(()=>{"use strict";ne(),ie(),se(),js(),Rl=e=>{let t=e[0].dataType,r=O.size(e[0].dims),n=O.size(e[1].dims),i=n%4===0,s=a=>{let u=R("x",t,[1],4),l=R("bias",t,[1],4),d=Z("y",t,[1],4),p=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],h=y=>`
  ...
  ${g.mainStart()}
    ...
    }`};return{name:"GatherBlockQuantized",shaderCache:{hint:`${t.cacheKey};${e.filter((y,_)=>_!==1).map(y=>y.dims.join("_")).join(";")}`,inputDependencies:Array.from({length:e.length},(y,_)=>"rank")},getRunData:()=>({outputs:[{dims:u,dataType:d}],dispatchGroup:{x:Math.ceil(l/64)},programUniforms:h}),getShaderSource:g}},qh=(e,t)=>{let r=e.inputs;Ul(r,t),e.compute(Ll(e.inputs,t))},Gh=e=>ye({blockSize:e.blockSize,gatherAxis:e.gatherAxis,quantizeAxis:e.quantizeAxis})}),__=L(()=>{"use strict";ne(),ie(),Me(),se(),Fl=e=>{if(!e||e.length!==2)throw new Error("GatherElements requires 2 inputs.");if(e[0].dims.length<1)throw new Error("GatherElements requires that the data input be rank >= 1.");if(e[0].dims.length!==e[1].dims.length)throw new Error(`GatherElements requires that the data input and
...
`)}
  ...
  ${I.mainStart()}
    ...
    ${w?"inv_std_output[global_idx] = inv_std_dev":""};
        ...
        }`};return{name:"BlockwiseMatMulNBits32",shaderCache:{hint:`${t.blockSize};${h};${g};${k};${b}`,inputDependencies:Array(e.length).fill("rank")},getRunData:()=>({outputs:[{dims:y,dataType:p}],dispatchGroup:{x:S},programUniforms:I}),getShaderSource:B}},df=(e,t)=>{fd(e.inputs,t),t.blockSize===32&&e.adapterInfo.isVendor("intel")&&e.adapterInfo.isArchitecture("gen-12lp")?e.compute(gd(e.inputs,t)):e.compute(md(e.inputs,t))},cf=e=>ye(e)}),S_=L(()=>{"use strict";ne(),ie(),se(),_d=e=>{if(!e||e.length<1)throw new Error("Too few inputs");if(e[0].dataType!==1&&e[0].dataType!==10)throw new Error("Input type must be float or float16.");if(e.length>=2){let t=e[0].dims.length*2===e[1].dims[0];if(e.length===4&&(t=e[3].dims[0]*2===e[1].dims[0]),!t)throw new Error("The pads should be a 1D tensor of shape [2 * input_rank] or [2 * num_axes].")}},yd=(e,t,r)=>{let n="";for(let i=t-1;i>=0;--i)n+=`
    ...
    `,u="",l=R("x",t.dataType,t.dims.length),d=["rank"],[p,h,g,y,_]=es(s,i);return p.push(...te(t.dims,s)),{name:e,shaderCache:{hint:`${n.cacheKey};${g};${y};${_}`,inputDependencies:d},getRunData:()=>({outputs:[{dims:s,dataType:t.dataType}],dispatchGroup:{x:Math.ceil(O.size(s)/64)},programUniforms:p}),getShaderSource:b=>ts(b,l,t.dims.length,s.length,i,a,u,t.dataType===10?-65504:-1e5,h,g,y,_)}},_f=(e,t)=>{jr(e.inputs),e.compute(as("MaxPool",e.inputs[0],!1,t))},yf=e=>{let t=e.storage_order,r=e.dilations,n=ns(e);if(t!==0)throw new Error("column major storage order is not yet supported for MaxPool");if(n.ceilMode!==0)throw new Error("using ceil() in shape computation is not yet supported for MaxPool");let i={storageOrder:t,dilations:r,...n,cacheKey:""};return{...i,cacheKey:Td(i)}},wf=e=>{let t=e.format;return{format:t,...ss,cacheKey:t}},bf=(e,t)=>{jr(e.inputs),e.compute(as("GlobalMaxPool",e.inputs[0],!0,t))}}),I_=L(()=>{"use strict";ne(),ie(),Me(),se(),Id=(e,t)=>{if(e.length<2||e.length>3)throw new Error("DequantizeLinear requires 2 or 3 inputs.");if(e.length===3&&e[1].dims===e[2].dims)throw new Error("x-scale and x-zero-point must have the same shape.");if(e.length===3&&e[0].dataType!==e[2].dataType)throw new Error("x and x-zero-point must have the same data type.");if(e[0].dataType===6&&e.length>2)throw new Error("In the case of dequantizing int32 there is no zero point.");if(e[1].dims.length!==0&&e[1].dims.length!==1&&e[1].dims.length!==e[0].dims.length)throw new Error("scale input must be a scalar, a 1D tensor, or have the same rank as the input tensor.");if(e.length>2){if(e[0].dataType!==e[2].dataType)throw new Error("x and x-zero-point must have the same data type.");if(e[1].dims.length!==e[2].dims.length)throw new Error("scale and zero-point inputs must have the same rank.");if(!e[1].dims.map((r,n)=>r===e[2].dims[n]).reduce((r,n)=>r&&n,!0))throw new Error("scale and zero-point inputs must have the same shape.")}if(t.blockSize>0){if(e[1].dims.length===0||e[1].dims.length===1&&e[1].dims[0]===1)throw new Error("blockSize must be set only for block quantization.");if(!e[1].dims.map((i,s)=>s===t.axis||i===e[0].dims[s]).reduce((i,s)=>i&&s,!0))throw new Error("For block qunatization, scale input shape to match the input shape except for the axis");if(e[1].dims.length!==e[0].dims.length)throw new Error("For block qunatization the scale input rank must be the same as the x rank.");let r=e[0].dims[t.axis],n=e[1].dims[t.axis];if(t.blockSize<Math.ceil(r/n)||t.blockSize>Math.ceil(r/(n-1)-1))throw new Error("blockSize must be with in the range [ceil(dI / Si), ceil(dI / (Si - 1) - 1)].")}},Ed=(e,t)=>{let r=O.normalizeAxis(t.axis,e[0].dims.length),n=e[0].dataType,i=n===3,s=e[0].dims,a=e[1].dataType,u=O.size(s),l=n===3||n===2,d=l?[Math.ceil(O.size(e[0].dims)/4)]:e[0].dims,p=e[1].dims,h=e.length>2?e[2]:void 0,g=h?l?[Math.ceil(O.size(h.dims)/4)]:h.dims:void 0,y=p.length===0||p.length===1&&p[0]===1,_=y===!1&&p.length===1,b=Ae(u),k=y&&(!l||b===4),v=k?b:1,w=k&&!l?b:1,T=R("input",l?12:n,d.length,w),S=R("scale",a,p.length),I=h?R("zero_point",l?12:n,g.length):void 0,C=Z("output",a,s.length,v),A=[T,S];I&&A.push(I);let $=[d,p];h&&$.push(g);let B=[{type:12,data:u/v},{type:12,data:r},{type:12,data:t.blockSize},...te(...$,s)],U=H=>{let W=[{name:"output_size",type:"u32"},{name:"axis",type:"u32"},{name:"block_size",type:"u32"}];return`
      ...
      }`};return{name:"DequantizeLinear",shaderCache:{hint:t.cacheKey,inputDependencies:I?["rank","rank","rank"]:["rank","rank"]},getShaderSource:U,getRunData:()=>({outputs:[{dims:s,dataType:a}],dispatchGroup:{x:Math.ceil(u/v/64),y:1,z:1},programUniforms:B})}},$f=(e,t)=>{Id(e.inputs,t),e.compute(Ed(e.inputs,t))},vf=e=>ye({axis:e.axis,blockSize:e.blockSize})}),E_=L(()=>{"use strict";it(),ne(),se(),Cd=(e,t,r)=>{let n=e===t,i=e<t&&r<0,s=e>t&&r>0;if(n||i||s)throw new Error("Range these inputs' contents are invalid.")},zd=(e,t,r,n)=>{let i=Math.abs(Math.ceil((t-e)/r)),s=[i],a=i,u=[{type:12,data:a},{type:n,data:e},{type:n,data:r},...te(s)],l=d=>{let p=Z("output",n,s.length),h=p.type.value,g=[{name:"outputSize",type:"u32"},{name:"start",type:h},{name:"delta",type:h}];return`
        ${d.registerUniforms(g).declareVariables(p)}
        ${d.mainStart()}
        ${d.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.outputSize")}
        output[global_idx] = uniforms.start + ${h}(global_idx) * uniforms.delta;
      }`};return{name:"Range",shaderCache:{hint:`${n}`},getShaderSource:l,getRunData:()=>({outputs:[{dims:s,dataType:n}],dispatchGroup:{x:Math.ceil(a/64)},programUniforms:u})}},xf=e=>{let t=0,r=0,n=0;e.inputs[0].dataType===6?(t=e.inputs[0].getInt32Array()[0],r=e.inputs[1].getInt32Array()[0],n=e.inputs[2].getInt32Array()[0]):e.inputs[0].dataType===1&&(t=e.inputs[0].getFloat32Array()[0],r=e.inputs[1].getFloat32Array()[0],n=e.inputs[2].getFloat32Array()[0]),ke.webgpu.validateInputContent&&Cd(t,r,n),e.compute(zd(t,r,n,e.inputs[0].dataType),{inputs:[]})}}),C_=L(()=>{"use strict";ne(),ie(),Me(),se(),Ad=(e,t,r,n)=>{if(e!=="none"&&n!=="i32"&&n!=="u32"&&n!=="f32")throw new Error(`Input ${n} is not supported with reduction ${e}.`);let i=`{
                var oldValue = 0;
                loop {
                  let newValueF32 =`,s=`;
  ...
  for (var i = 0u; i < uniforms.num_updates_elements; i++) {
    ...
    ${Ad(t.reduction,"output[data_offset + i]","value",_.type.value)}
      ...
      }`};return{name:"ScatterND",shaderCache:{hint:`${t.cacheKey}_${t.reduction}`,inputDependencies:["rank","rank"]},getRunData:()=>({outputs:[{dims:i,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(a/64)},programUniforms:d}),getShaderSource:p}},kf=e=>ye({reduction:e.reduction}),Sf=(e,t)=>{e.compute(Md(e.inputs,t),{inputs:[e.inputs[1],e.inputs[2]],outputs:[]})}}),z_=L(()=>{"use strict";ne(),ie(),Me(),se(),Od=(e,t)=>{if(e.every(r=>r>0||(()=>{throw new Error("Resize requires scales input values to be positive")})),e.length>0){if(t.mode==="linear"){if(!(e.length===2||e.length===3||e.length===4&&e[0]===1&&e[1]===1||e.length===4&&e[0]===1&&e[3]===1||e.length===5&&e[0]===1&&e[1]===1))throw new Error(`For linear mode, Resize requires scales to be 2D, 3D, 4D with either two outermost or one innermost and
...
`}
      ...
      }`;return{name:"Slice",shaderCache:{hint:`${l.length}_${a.length}_${s.length}`,inputDependencies:["rank"]},getShaderSource:k,getRunData:()=>({outputs:[p],dispatchGroup:{x:Math.ceil(n/64)},programUniforms:b})}},Cf=(e,t)=>{Zd(e.inputs,t);let r=Yd(e.inputs,t);e.compute(ec(e.inputs,r),{inputs:[0]})},zf=e=>{let t=e.starts,r=e.ends,n=e.axes;return ye({starts:t,ends:r,axes:n})}}),O_=L(()=>{"use strict";ne(),ie(),Me(),Yt(),se(),tc=e=>{if(!e||e.length!==1)throw new Error("Softmax op requires 1 input.")},rc=(e,t)=>{let r=e.inputs[0],n=r.dims,i=O.size(n),s=n.length,a=O.normalizeAxis(t.axis,s),u=a<n.length-1,l,d=[];u?(d=Array.from({length:s},(A,$)=>$),d[a]=s-1,d[s-1]=a,l=e.compute(Je(r,d),{inputs:[r],outputs:[-1]})[0]):l=r;let p=l.dims,h=p[s-1],g=i/h,y=Ae(h),_=h/y,b=64;g===1&&(b=256);let k=(A,$)=>$===4?`max(max(${A}.x, ${A}.y), max(${A}.z, ${A}.w))`:$===2?`max(${A}.x, ${A}.y)`:$===3?`max(max(${A}.x, ${A}.y), ${A}.z)`:A,v=R("x",l.dataType,l.dims,y),w=Z("result",l.dataType,l.dims,y),T=v.type.value,S=De(l.dataType)==="f32"?`var threadMax = ${T}(-3.4028234663852886e+38f);`:`var threadMax = ${T}(-65504.0h);`,I=A=>`
