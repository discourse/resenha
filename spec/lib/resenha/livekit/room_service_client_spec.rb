...
RSpec.describe Resenha::Livekit::RoomServiceClient do
  before do
    ...
    it "POSTs the room name and identity with an admin bearer token" do
      ...
      expect(
        a_request(
          ...
          body == { "room" => Resenha::Livekit.room_name(room), "identity" => user.id.to_s } &&
            req.headers["Content-Type"] == "application/json" && claims["iss"] == "lk_api_key" &&
            claims["video"] == { "roomAdmin" => true, "room" => Resenha::Livekit.room_name(room) }
        end,
  ...
  context "when LiveKit is down" do
    it "never raises on a timeout and logs the failure" do
      ...
      Rails.logger.expects(:warn).with(regexp_matches(/\[resenha-livekit\] DeleteRoom failed/))
    ...
    it "never raises on an error response and logs the failure" do
      ...
      Rails
        .logger
        .expects(:warn)
        .with(regexp_matches(/\[resenha-livekit\] RemoveParticipant failed.*HTTP 500/))
