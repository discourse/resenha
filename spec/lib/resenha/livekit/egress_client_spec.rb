...
RSpec.describe Resenha::Livekit::EgressClient do
  before do
  ...
  describe ".start_room_composite" do
    it "POSTs the room name and file output with a roomRecord-granted token" do
      ...
      expect(
        a_request(
          ...
          body ==
            {
              "roomName" => Resenha::Livekit.room_name(room),
              "audioOnly" => true,
              "fileOutputs" => [{ "filepath" => "resenha/test-abc123" }],
    ...
    it "requests a full composite for a room with video enabled" do
      ...
      described_class.start_room_composite(room, filepath: "resenha/test-abc123")
