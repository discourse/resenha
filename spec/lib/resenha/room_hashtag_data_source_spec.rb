...
require_relative "../../../db/migrate/20260220154336_add_cooked_description_to_resenha_rooms"
...
RSpec.describe Resenha::RoomHashtagDataSource do
  before do
    ActiveRecord::Migration.suppress_messages do
      unless ActiveRecord::Base.connection.table_exists?(:resenha_rooms)
        CreateResenhaRooms.new.change
      end
      unless ActiveRecord::Base.connection.column_exists?(:resenha_rooms, :cooked_description)
        AddCookedDescriptionToResenhaRooms.new.change
      end
    ...
    SiteSetting.resenha_allowed_groups = Group::AUTO_GROUPS[:trust_level_1].to_s
  ...
  fab!(:room) do
    Fabricate(:resenha_room, name: "General", description: "Come hang out", public: true, creator:)
  end
  fab!(:private_room) { Fabricate(:resenha_room, name: "Secret Sessions", creator:) }
  fab!(:stage_room) do
    Fabricate(
      :resenha_room,
      name: "Town Hall",
  ...
  describe "#enabled?" do
    it "follows the resenha_enabled setting" do
  ...
  describe "cooking" do
    it "cooks a room hashtag into a link for a user who can see the room" do
      ...
      expect(link["href"]).to eq("/resenha/r/general")
  ...
  describe "chat hashtag configuration snapshot" do
    it "tracks the resenha_enabled setting at runtime" do
