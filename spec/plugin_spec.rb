...
RSpec.describe UserDestroyer do
  describe "resenha cleanup" do
    fab!(:admin)
    fab!(:user)
    fab!(:other_user, :user)
    fab!(:room) { Fabricate(:resenha_room, creator: user, public: true) }
    ...
    it "removes memberships and co-presences but keeps sessions as history" do
      ...
      expect(Resenha::RoomMembership.where(user_id: user.id)).to be_empty
      expect(Resenha::CoPresence.exists?(co_presence.id)).to eq(false)
