...
RSpec.describe Resenha::AdminLivekitController do
  before do
  ...
  after do
    ...
    Discourse.redis.del(Resenha::Livekit::HealthCheck::LAST_PROBE_KEY)
  ...
  describe "when LiveKit is configured" do
    ...
    it "surfaces the stored probe result and the webhook freshness marker" do
      list_rooms_stub.to_return(status: 200, body: { rooms: [] }.to_json)
      freeze_time
      Resenha::Livekit.touch_last_webhook!
      Jobs::ResenhaLivekitProbe.new.execute({})
