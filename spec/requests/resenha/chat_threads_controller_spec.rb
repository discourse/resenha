...
RSpec.describe Resenha::ChatThreadsController do
  before do
  ...
  fab!(:room) { Fabricate(:resenha_room, public: true, chat_channel_id: channel.id) }
  ...
  before do
    ...
    SiteSetting.resenha_create_room_allowed_groups = Group::AUTO_GROUPS[:trust_level_4].to_s
  end

  def mark(chat_thread, resenha_room)
    chat_thread.upsert_custom_fields(Resenha::THREAD_ROOM_ID_FIELD => resenha_room.id)
    chat_thread
  end
  ...
  describe "#show" do
    it "requires authentication" do
      ...
      get "/resenha/chat_threads/#{thread.id}.json"
    ...
    context "when signed in" do
      ...
      it "reports a thread that is still the room's live session as active" do
        ...
        Discourse.redis.set("resenha:room:#{room.id}:chat_thread", thread.id)
      ...
      it "reports a thread whose session rolled over to a newer thread as inactive" do
        ...
        Discourse.redis.set("resenha:room:#{room.id}:chat_thread", newer_thread.id)
      ...
      it "returns 404 for a thread id that does not exist" do
        get "/resenha/chat_threads/#{Chat::Thread.maximum(:id).to_i + 1}.json"
      ...
      it "returns 404 when the marker points at a room linked to a different channel" do
        ...
        other_room = Fabricate(:resenha_room, public: true, chat_channel_id: other_channel.id)
      ...
      it "returns 404 for a marked thread whose room the user cannot see" do
        private_room = Fabricate(:resenha_room, public: false, chat_channel_id: channel.id)
      ...
      it "returns 404 when the user cannot preview the thread's channel" do
        ...
        visible_room = Fabricate(:resenha_room, public: true, chat_channel_id: private_channel.id)
        mark(private_thread, visible_room)

        get "/resenha/chat_threads/#{private_thread.id}.json"

        expect(response.status).to eq(404)
      end

      it "returns 404 when resenha chat is disabled" do
        SiteSetting.resenha_chat_enabled = false
        mark(thread, room)
