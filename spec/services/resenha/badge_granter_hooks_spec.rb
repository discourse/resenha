...
RSpec.describe Resenha::BadgeGranterHooks do
  fab!(:user) { Fabricate(:user, trust_level: TrustLevel[2]) }
  fab!(:room, :resenha_room) { Fabricate(:resenha_room, public: true, max_participants: 3) }

  before do
    SiteSetting.resenha_enabled = true
    SiteSetting.resenha_badges_enabled = true
    SiteSetting.resenha_analytics_enabled = true
    SeedFu.seed(Rails.root.join("plugins/resenha/db/fixtures"))
    described_class.enable_all!
  end

  describe ".on_leave" do
    def build_session(joined_at:, left_at:)
      Resenha::Session.create!(user: user, room: room, joined_at: joined_at, left_at: left_at)
    end

    describe "Mic Check" do
      it "grants when session is 30+ seconds and others are in the room" do
        other = Fabricate(:user)
        Resenha::ParticipantTracker.add(room.id, other.id)
    ...
    it "does nothing when session has no left_at" do
      session = Resenha::Session.create!(user: user, room: room, joined_at: 5.hours.ago)
      described_class.on_leave(user, session, room: room)
  ...
  describe ".on_join" do
    ...
    describe "Icebreaker" do
      ...
      it "grants when user has no co-presence history with anyone in the room" do
        Resenha::ParticipantTracker.add(room.id, stranger.id)
  ...
  describe "fixture seeding" do
    ...
    it "creates the Resenha badge grouping" do
      expect(BadgeGrouping.exists?(name: "Resenha")).to eq(true)
    end

    it "is idempotent" do
      expect { SeedFu.seed(Rails.root.join("plugins/resenha/db/fixtures")) }.not_to change {
        Badge.count
      }
    end

    it "sets SQL queries on scheduled badges" do
      expect(Badge.find_by(name: "Rookie").query).to include("resenha_sessions")
      expect(Badge.find_by(name: "Social Butterfly").query).to include("resenha_co_presences")
    end
    ...
    it "allows gold badges to be used as title" do
      gold_badges = resenha_badges.where(badge_type_id: BadgeType::Gold)
  ...
  describe ".enable_all!" do
    ...
    it "enables all Resenha badges" do
      described_class.enable_all!

      resenha_badges = Badge.joins(:badge_grouping).where(badge_groupings: { name: "Resenha" })
      expect(resenha_badges.where(enabled: false).count).to eq(0)
    end
  end

  describe ".disable_all!" do
    it "disables all Resenha badges" do
      described_class.disable_all!
