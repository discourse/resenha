...
RSpec.describe Jobs::ResenhaLivekitProbe do
  before do
  ...
  after { Resenha::Livekit::HealthCheck.clear_probe! }
  ...
  it "stores a passing result when the server is reachable" do
    ...
    probe = Resenha::Livekit::HealthCheck.last_probe
  ...
  it "stores the failure and logs it with the [resenha-livekit] prefix" do
    list_rooms_stub.to_timeout
    Rails
      .logger
      .expects(:warn)
      .with(regexp_matches(/\[resenha-livekit\] connectivity probe failed/))
  ...
  it "drops the stored result instead of probing when LiveKit was deconfigured" do
    Resenha::Livekit::HealthCheck.store_probe!(ok: true)
    SiteSetting.resenha_livekit_api_secret = ""
    probe_stub = stub_request(:post, %r{\Ahttps://})

    described_class.new.execute({})

    expect(Resenha::Livekit::HealthCheck.last_probe).to be_nil
  ...
  describe "enqueueing" do
    it "is enqueued when a LiveKit setting changes" do
      expect_enqueued_with(job: :resenha_livekit_probe) do
        SiteSetting.resenha_livekit_url = "wss://other.example.com"
      end
    end
