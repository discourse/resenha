...
Ephemeral rooms are short-lived `Resenha::Room` records created by **other
...
- **Never appears in discovery surfaces**: the room directory (`GET
  /resenha/rooms`), directory MessageBus broadcasts on
  `/resenha/rooms/index`, and hashtag autocomplete/rendering all exclude it.
...
- **Cannot be created through the rooms API**: the `ephemeral` attribute is
  ...
  way in is `Resenha::EphemeralRoomManager.create!`.
...
manage). The room page at `/resenha/r/:slug` renders it, so consumers can
...
- Any other `Resenha::Room` attribute (`public`, `video_enabled`,
...
- Rooms empty for longer than `resenha_ephemeral_room_ttl_minutes`
...
`Resenha::EphemeralRoomManager.destroy!(room)`, which performs the same full
teardown.

## Telling them apart

- Ruby: `room.ephemeral?`, and the `Resenha::Room.ephemeral` /
  `Resenha::Room.persistent` scopes.
- Serialized rooms (`Resenha::RoomSerializer`) carry an `ephemeral`
