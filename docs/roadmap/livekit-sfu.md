...
`resenha-webrtc.js` — not as a formal transport-interface refactor. The mesh
...
The exact call sites in `resenha-webrtc.js` that branch on transport.
...
`lib/resenha/peer-manager.js`, `lib/resenha/signaling.js`,
...
settings modals, sidebar), all initializers, `resenha-rooms.js`, and the whole
...
```yaml
resenha_livekit_url:            # wss://livekit.example.com — empty = unavailable
  default: ""
...
resenha_livekit_room_policy:
  ...
  validator: "ResenhaLivekitPolicyValidator"
resenha_livekit_room_prefix:    # LiveKit room-name namespace override
  default: ""
resenha_livekit_mesh_fallback:  # opt-in: empty room + mint failure → pin mesh
  default: false
```

None are `client: true`. `ResenhaLivekitPolicyValidator` follows core's
`lib/validators/*_validator.rb` convention: refuses any policy other than
`disabled` unless `resenha_livekit_url` (must be `ws://` or `wss://`),
`resenha_livekit_api_key`, and `resenha_livekit_api_secret` are all present,
with a specific translated error per missing field (`server.en.yml`). Admins
get config-time failure, not first-join failure.

`plugin.rb` serializes one Site attribute next to `resenha_public_access`
(plugin.rb:90): `resenha_livekit_per_room_available` = configured **and**
policy == `per_room`. It exists solely to gate the room-form checkbox — the
client never needs the policy enum itself.

### New: `lib/resenha/livekit.rb`

```ruby
module Resenha::Livekit
  ...
  def self.room_name(room)
    ...
    # (room id, not slug — slugs are mutable); resenha_livekit_room_prefix overrides the db prefix
  end
...
- `video`: `room` = room_name, `roomJoin: true`, `canSubscribe: true`,
  `canPublish` = `guardian.can_speak_in_resenha_room?(room)`
...
`ensure_can_join_resenha_room!`. `resenha_video_max_publishers` stays enforced
where it is today (`rooms_controller#state`, rooms_controller.rb:207–210) —
it's a roster-flag gate, transport-independent.

### Transport pin: `app/services/resenha/participant_tracker.rb`

(NOT `lib/resenha/` — the class lives in app/services, `KEY_NAMESPACE =
"resenha:room"`, `SAFETY_TTL = 30.minutes` at :5–7.)

- New Redis key `resenha:room:{id}:transport`; `pin_transport!(room_id,
  transport)` uses `SET NX`; `pinned_transport(room_id)`;
  `clear_transport_pin(room_id)`.
- TTL = `2 × SiteSetting.resenha_participant_ttl_seconds` (settings.yml:19,
...
### `app/controllers/resenha/rooms_controller.rb`
...
- **New `#livekit_token`** (`POST rooms/:id/livekit_token` in
  `config/routes.rb`): `ensure_can_join_resenha_room!` +
  `RateLimiter.new(current_user, "resenha-livekit-token", 10, 1.minute)`
...
### `app/controllers/resenha/room_memberships_controller.rb`
...
### New: `lib/resenha/livekit/room_service_client.rb` (~60 lines)
...
`rake resenha:clear_transport_pins` for emergencies.
...
2. `GET /admin/plugins/resenha/livekit/status` (AdminConstraint) returns:
...
3. Rendered as a card in `admin/assets/javascripts/admin/components/resenha-dashboard.gjs`.
4. A request spec asserts the API secret never appears in any serializer or
   status payload.

### Per-room policy surface

- Migration: `add_column :resenha_rooms, :livekit_enabled, :boolean,
...
- `components/resenha-room-form.gjs`: one FormKit checkbox ("Use media server
  (SFU)") gated on `Site.resenha_livekit_per_room_available`. Stale-form edge:
...
`POST /resenha/livekit/webhook` (engine-scoped, skips CSRF + login), verifies
...
`SETEX`es `resenha:livekit:last_webhook_at` for the status panel (stale = warning,
...
### New: `lib/resenha/livekit-session.js` (~350 lines)
...
Default `loadSdk`:
`import(getURL("/plugins/resenha/javascripts/livekit/livekit-client.mjs"))` —
...
### Modified: `app/services/resenha-webrtc.js` (~200-line diff, all additive/guarded)
...
2. `join(room)` (~:357): read `data.transport ?? "mesh"` and `data.livekit`
   ...
   different from the server-side mint-failure copy) + `[resenha-livekit]`
...
```
pinned = redis GET resenha:room:{id}:transport
...
- **Config changes affect only the next room instance** (first join after the
  ...
  `rake resenha:clear_transport_pins`.

## 7. Failure-mode policy

| # | Failure | Behavior |
|---|---|---|
| a | Token mint fails, room EMPTY | Join fails: specific translated toast ("Voice server unavailable — contact your administrator"), `[resenha-livekit]` log, status panel red. If `resenha_livekit_mesh_fallback` is on (opt-in — silent degradation hides outages from ops) **and the room is empty**, pin mesh for this call and proceed. |
...
| f | Mint succeeds but the CLIENT can't reach the SFU (corporate firewall blocking WSS/UDP — the most common enterprise failure) | Distinct toast ("your network cannot reach the voice server"), immediate `DELETE /leave` cleanup, `[resenha-livekit]` console log. **No mesh fallback even under (a)'s setting** — other clients may reach the SFU fine. |
...
`resenha-webrtc.js`: `#roomTransports`/`#isMeshRoom` defaulting mesh; guard
...
*Acceptance*: entire existing JS suite (`resenha-webrtc-test.js`,
...
`settings.yml` + locales, `lib/validators/resenha_livekit_policy_validator.rb`,
`lib/resenha/livekit.rb`, pin helpers in
`app/services/resenha/participant_tracker.rb`, `rooms_controller.rb`
...
`lib/resenha/livekit-session.js`, `resenha-webrtc.js` (join/connect path with
...
`lib/resenha/livekit/room_service_client.rb` (3 Twirp calls),
...
clear), `rake resenha:clear_transport_pins`, WebMock specs.
...
both param permit lists, `resenha-room-form.gjs` FormKit checkbox gated on
`Site.resenha_livekit_per_room_available`, locales.
...
`spec/system/resenha_livekit_spec.rb` (two-browser camera flow mirroring
`resenha_voice_rooms_spec.rb`, gated on `ENV["RESENHA_LIVEKIT_TEST_URL"]`,
docs show `docker run livekit/livekit-server --dev`); verify
`resenha_fake_media.rb` fakes satisfy the SDK (expected no-op — livekit-client
...
`isBrowserSupported()` guard wired to a translated toast; resenha-bots README
...
dashboard card, `[resenha-livekit]` log audit, secret-never-serialized spec,
per §4.
...
1. Provision: 2–4 cores is ample (audio ≈ hundreds of participants/core,
   ...
   `webhook.urls: [https://forum.example/resenha/livekit/webhook]` if PR 8 is
...
2. Network: reverse proxy (WebSocket upgrade + long timeouts) → 7880 as
   ...
   risk 8).
3. Discourse: set `resenha_livekit_url` / `_api_key` / `_api_secret`; pick
   `resenha_livekit_room_policy`. Mesh rooms keep using the existing
   STUN/TURN settings.
...
5. Operations: settings changes apply to the next room instance; force-flip a
   live room via "End live call" or `rake resenha:clear_transport_pins`. If
...
3. **Watcher-gating semantics shift** — mesh gates at the publisher
   ...
   `resenha_video_max_publishers` copy ("increases CPU for everyone") is wrong
...
8. **Core CSP roadmap** — core plans to promote `connect_src` to an
   ...
   `resenha_livekit_url` host or every LiveKit install breaks on a core
   upgrade. Track it; cheap now, expensive to discover later.

## 12. Open questions (product)

1. Should `all_rooms` also cover audio-only rooms while `resenha_video_enabled`
...
4. resenha-bots at SFU scale: a lone bot *does* transmit to an SFU (the mesh
