...
By default Resenha runs calls as a pure peer-to-peer mesh: every participant
...
- `resenha_livekit_room_policy` controls the resolution: `disabled` (default,
...
| Setting | Value |
| --- | --- |
| `resenha_livekit_url` | `wss://livekit.example.com` (or `ws://` for plain-HTTP labs) |
| `resenha_livekit_api_key` | the API key |
| `resenha_livekit_api_secret` | the API secret |
| `resenha_livekit_room_policy` | `per_room` or `all_rooms` |
| `resenha_livekit_room_prefix` | optional room-name namespace; defaults to the site's database name |
| `resenha_livekit_mesh_fallback` | opt-in: when a token can't be minted and the room is empty, start on mesh instead of failing the join. Off by default — silent degradation hides outages |
...
Mesh rooms keep using the existing `resenha_stun_servers` /
`resenha_turn_servers` settings; LiveKit brings its own ICE/TURN and ignores
them.
...
3. The join response (`POST /resenha/rooms/:id/join` in devtools) carries
   `"transport": "livekit"`.
...
prefixed `[resenha-livekit]`. If joins fail with "Your network cannot reach
...
- **Ending a live call**: admins can end any room's call from
  **Admin → Plugins → Resenha** (the room list's "End call" action). It kicks
...
- **Emergency levers**:
  - `rake resenha:clear_transport_pins` drops every pinned transport at once
...
(`public/javascripts/livekit/livekit-client.js`, rebuilt via
`scripts/build-livekit-bundle.sh`). Server upgrades within the same major
