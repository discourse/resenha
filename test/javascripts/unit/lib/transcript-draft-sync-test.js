...
import TranscriptDraftSync from "discourse/plugins/resenha/discourse/lib/resenha/transcript-draft-sync";

module("Resenha | Unit | Lib | transcript-draft-sync", function (hooks) {
  setupTest(hooks);
  ...
  test("flushes only when dirty and tracks the sequence", async function (assert) {
    ...
    assert.strictEqual(this.sync.key, "new_topic_resenha_5_1000");
  ...
  test("stop flushes what is pending", async function (assert) {
    ...
    assert.strictEqual(this.saves.length, 1);
    assert.strictEqual(this.sync.key, "new_topic_resenha_5_1000", "key kept");
  });
});
