...
export default class ResenhaDashboardRoute extends Route {
  queryParams = {
  ...
  async model() {
    const controller = this.controllerFor(
      "admin-plugins.show.resenha-dashboard"
    );
    const queryString = this.#buildQuery(controller);

    const [overview, rooms, users] = await Promise.all([
      ajax(`/admin/plugins/resenha/stats/overview.json?${queryString}`),
      ajax(`/admin/plugins/resenha/stats/rooms.json?${queryString}`),
      ajax(`/admin/plugins/resenha/stats/users.json?${queryString}`),
    ]);
