...
module Resenha
  ...
  class ReviewQueue
    def flag_user(room, target_user, guardian, flag_type_id, opts = {})
      ...
      guardian.ensure_can_flag_resenha_user!(room, target_user)

      session = Resenha::Session.where(room: room, user: target_user).order(joined_at: :desc).first

      if session.blank?
        result[:errors] << I18n.t("resenha.errors.flag_target_never_joined")
        return result
      end
      ...
      if !can_flag_again?(existing_reviewable, guardian.user)
        result[:errors] << I18n.t("resenha.errors.flag_already_handled")
        return result
      end
      ...
      reviewable =
        ReviewableResenhaUser.needs_review!(
    ...
    def companion_pm_creator(room, target_user, flagger, opts)
      title =
        I18n.t(
          "resenha.reviewable_score_types.notify_moderators.pm_title",
      ...
      body =
        I18n.t(
          "resenha.reviewable_score_types.notify_moderators.pm_body",
          message: opts[:message],
          username: target_user.username,
          room_name: room.name,
          link: "#{Discourse.base_url}/resenha/r/#{room.slug}",
          locale: SiteSetting.default_locale,
        )
