  ...
  class RecordingManager
    ...
    class << self
      def available_for?(room)
        SiteSetting.resenha_livekit_recording_enabled &&
          Resenha::ParticipantTracker.pinned_transport(room.id) == "livekit"
      end

      # Public shape shared by the serializer, the broadcasts, and the
      # start endpoint's response.
      def status(room_id)
        info = Resenha::ParticipantTracker.recording(room_id)
      ...
      def start!(room, user)
        raise Error, I18n.t("resenha.errors.recording_unavailable") unless available_for?(room)
        if Resenha::ParticipantTracker.recording(room.id)
          raise Error, I18n.t("resenha.errors.recording_already_active")
        end
        ...
        filepath = "#{SiteSetting.resenha_livekit_recording_filepath}-#{SecureRandom.hex(8)}"
      ...
      def stop!(room)
        info = Resenha::ParticipantTracker.recording(room.id)
        raise Error, I18n.t("resenha.errors.recording_not_active") if info.nil?

        result = Resenha::Livekit::EgressClient.stop(info[:egress_id])
        ...
        raise Error, I18n.t("resenha.errors.recording_stop_failed") unless result[:ok]

        Resenha::ParticipantTracker.clear_recording(room.id)
        broadcast(room)
      end
      ...
      def reconcile_pending!
        Resenha::Recording
          ...
          .find_each do |recording|
            result = Resenha::Livekit::EgressClient.list(egress_id: recording.egress_id)
      ...
      def deliver_recording(recording)
        ...
        body_key =
          if recording.location.present?
            "resenha.recording_ready_pm.body_with_link"
          else
            "resenha.recording_ready_pm.body_with_path"
          end
        ...
        PostCreator.create!(
          ...
          title: I18n.t("resenha.recording_ready_pm.title", room_name: recording.room.name),
          raw:
            I18n.t(
              body_key,
        ...
        Rails.logger.warn(
          "[resenha-livekit] recording PM failed for recording #{recording.id}: " \
            "#{e.class} #{e.message}",
        )
      end

      def format_duration(duration_ms)
        return I18n.t("resenha.recording_ready_pm.unknown_duration") if duration_ms.blank?
      ...
      def broadcast(room)
        Resenha::RoomBroadcaster.new(room).publish_room(
